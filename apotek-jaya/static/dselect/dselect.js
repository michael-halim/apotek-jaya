...
function dselectSearch(event, input, classElement, classToggler, creatable) {
  ...
  const found = Array.from(items).filter(
    (i) => !i.classList.contains('d-none') && !i.hasAttribute('hidden')
  );
  if (found.length < 1) {
    ...
    if (creatable) {
      ...
      if (event.key === 'Enter') {
        ...
        target.insertAdjacentHTML(
          'afterbegin',
          `<option value="${input.value}" selected>${input.value}</option>`
        );
        target.dispatchEvent(new Event('change'));
        input.value = '';
...
function dselect(el, option = {}) {
  ...
  const creatable =
    attrBool('creatable') || option.creatable || defaultCreatable;
  const clearable =
    attrBool('clearable') || option.clearable || defaultClearable;
  const maxHeight =
    el.dataset.dselectMaxHeight || option.maxHeight || defaultMaxHeight;
  ...
  function createDom() {
    ...
    <div class="dropdown ${classElement} ${additionalClass}">
      <button class="${classToggler} ${
      !el.multiple && clearable ? classTogglerClearable : ''
    }" data-dselect-text="${
